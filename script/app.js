...
function renderTasks(tasks){
  ...
  if(ALL_TASKS.length > 0){
    tasks.forEach((taskItem, index) => {
      ...
      <small>${taskItem.isEdited ? "Edited" : ""}</small>
      <strong ${taskItem.showEdit ? `contenteditable="true"`:`contenteditable="false"`} class="${taskItem.isCompleted ? "completed-item" : ""}">${taskItem.taskName}</strong> 
        <div class="todolist__item-btns">
          <button class="complete-btn"> <i class="bi bi-check-circle-fill"></i> Complete </button>
          ${taskItem.showEdit ? `<button class="edit-btn_second"> <i class="bi bi-check-all"></i>Done</button>` : 
...
todolistTaskContainer.addEventListener("click", (e) => {
  ...
  if(e.target.closest(".delete-btn")){
    ...
    ALL_TASKS[indexOfItem].showEdit = true;
    renderTasks(ALL_TASKS);
  }
  else if(e.target.closest(".edit-btn_second")){
    ALL_TASKS[indexOfItem].showEdit = false;
    ALL_TASKS[indexOfItem].isEdited = true;
    ALL_TASKS[indexOfItem].taskName = e.target.closest(".edit-btn_second").parentElement.parentElement.children[1].textContent
    renderTasks(ALL_TASKS); 
  }
})
